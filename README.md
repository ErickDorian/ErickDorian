
```
                                          
 _____     _     _      _____             
|   __|___|_|___| |_   |  _  |___ ___ ___ 
|   __|  _| |  _| '_|  |   __| -_|   | .'|
|_____|_| |_|___|_,_|  |__|  |___|_|_|__,|

                                          
```

```cmd

Microsoft Windows [Versão 10.0.19045.4291]
(c) Microsoft Corporation. Todos os direitos reservados.

C:\Users\ErickDorian> systeminfo

[SYSTEMINFO]
> Nome do host: Erick Pena
> Idade: 26
> Formação: Análise e Desenvolvimento de Sistemas
> Área: Cybersecurity | SOC N1 / Blue Team
> Localização: Betim - Minas Gerais - Brasil
> Status: Em treinamento contínuo...
> Especialização: Redes e Segurança da Informação
> Certificações: Cisco Networking Academy, Udemy IT Security, Bradesco TI
> Idiomas: Português (nativo), Inglês (intermediário)
> Soft Skills: Trabalho em equipe, Resolução de problemas, Automação
> Objetivo: Migrar para área de Cybersecurity

```

```bash

C:\Users\ErickDorian> skills

[SKILLS]
> Redes: TCP/IP, DNS, DHCP, OSI, Wireshark, Packet Tracer
> Sistemas: Linux (Shell Script), Windows (Administração)
> Segurança: Nmap, Análise de Logs, Blue Team, SOC Monitoring
> Ferramentas: Git, GitHub, VirtualBox, VS Code, PowerShell
> Extras: Automação com Excel/Macros, Documentação Técnica

```

```powershell

C:\Users\ErickDorian> exp

[EXPERIENCE]
> Nacional Gás - Assistente de Logística
  - Automação de processos com planilhas e macros
  - Redução de falhas humanas → mindset de prevenção
  - Suporte ao cliente e resolução de problemas

> Grupo Funcional - Auxiliar de Logística
  - Emissão de notas fiscais e gestão de entregas
  - Organização e controle de dados → atenção a detalhes
  - Trabalho sob pressão em ambiente crítico

> Espro - Aprendiz
  - Gestão de documentos e conformidade com normas
  - Segurança do trabalho → foco em compliance
  - Trabalho em equipe e uso de Office 365

```

```yaml

C:\Users\ErickDorian> certs

[CERTIFICACOES]
> Udemy - IT Security Specialist
  - Cybersecurity, Information Security
  - Código: UC-9f26eb02-5de8-472a-b60c-c37b2baa16b2

> Udemy - Dominando Linux do Zero
  - Linux, Shell Script
  - Código: UC-d0ef79d6-17ef-4846-9955-26002d831067

> Udemy - Redes de Computadores
  - TCP/IP, Servidores, Protocolos
  - Código: UC-25ce78db-9f42-47d2-9443-56979f28c55e

> Fundação Bradesco - Fundamentos de TI
  - Hardware, Software, Infraestrutura
  - Código: CF112183-1AE1-4A22-B218-0A0E55270E39

> Cisco Networking Academy - Introdução à Cibersegurança
  - Segurança da Informação, Blue Team
  - Código: 8986cc89-6ea6-4ad4-bf51-4bda1019555a

```

```plaintext

C:\Users\ErickDorian> proj

[PROJECTS]
> Loading...
> Próximos commits virão em breve...

```

```ini
C:\Users\ErickDorian> ctt

[CONTACT]
> Email: erickspena@outlook.com.br
> LinkedIn: linkedin.com/in/ericksspena

```

```cmd
C:\Users\ErickDorian> exit

Encerrando sessão...

```


[![Email](https://img.shields.io/badge/Email-0078D4?style=for-the-badge&logo=microsoftoutlook&logoColor=white)](mailto:erickspena@outlook.com.br)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ericksspena/)

```



